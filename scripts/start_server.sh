...
if [[ "$WINWIN_ENV" = "Development" ]]; then
        ...
        nohup sudo java -Xmx4096m -Dserver.port=80 -javaagent:/home/ec2-user/elastic-apm-agent-1.12.0.jar -Delastic.apm.service_name=winwin-service-production -Delastic.apm.application_packages=com.winwin.winwin -Delastic.apm.server_urls=http://wiki.winwin.care:8200 -jar winwin-0.0.1-SNAPSHOT.jar /tmp 2>> /dev/null >> /dev/null &
fi
